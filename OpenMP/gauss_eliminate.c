...
void gauss_eliminate_using_omp(Matrix U)
{
    ...
    #pragma omp parallel num_threads(NUM_THREADS)
    {
        for (int k = 0; k < num_elements; k++) {
            if ((k % NUM_THREADS) == omp_get_thread_num()) {
            ...
            for (int i = (k + 1); i < num_elements; i++) {
                if ((i % NUM_THREADS) == omp_get_thread_num()) {
                    for (int l = k + 1; l < num_elements; l++) {
                        U.elements[num_elements * i + l] = (float)(U.elements[num_elements * i + l] - U.elements[num_elements * k + l] * U.elements[num_elements * i + k]);
                    }
    ...
    if (omp_get_thread_num() == NUM_THREADS - 1) {
        U.elements[(num_elements - 1) * num_elements + num_elements - 1] = 1;
    }
}
